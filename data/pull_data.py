...
def pull_quandl_sample_data(ticker: str) -> pd.DataFrame:
    return (
        ...
        .rename(
            columns={"Trade Date": "date", "Date": "date", "Settle": "close"}
        )
        .set_index("date")
        .replace(0.0, np.nan)
    )
...
def pull_pinnacle_data(ticker: str) -> pd.DataFrame:
    return pd.read_csv(
        os.path.join(
            PINNACLE_DATA_FOLDER, f"{ticker}_{PINNACLE_DATA_CUT}.CSV"
        ),
...
def pull_pinnacle_data_multiple(
    ...
    data = pd.concat(
        [
            pull_pinnacle_data(ticker).assign(ticker=ticker).copy()
    ...
    return (
        pd.concat(
            [
                _fill_blanks(
                    dates.merge(data.loc[t], on="date", how="left").assign(
                        ticker=t
                    )
                )
                for t in tickers
