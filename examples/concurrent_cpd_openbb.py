...
def wait_for_memory_to_clear(target_usage=MEMORY_WARNING - 10):
    ...
    while check_memory_usage() > target_usage:
        print(f"Waiting for memory to clear... Current: {check_memory_usage()}%")
...
def emergency_exit(processed_tickers, progress_file):
    ...
    print(f"Progress saved to {progress_file}. Run the script again to resume.")
    sys.exit(1)

def main(lookback_window_length: int, time_window_batch_size: int = TIME_WINDOW_BATCH_SIZE):
    ...
    if not os.path.exists(CPD_OPENBB_OUTPUT_FOLDER(lookback_window_length)):
        os.makedirs(CPD_OPENBB_OUTPUT_FOLDER(lookback_window_length), exist_ok=True)

    # Setup progress tracking
    progress_file = os.path.join(CPD_OPENBB_OUTPUT_FOLDER(lookback_window_length), "processed_tickers.txt")
    ...
    if os.path.exists(progress_file):
        ...
        print(f"Resuming from previous run. Already processed {len(processed_tickers)} tickers")
    
    # Filter out already processed tickers
    remaining_tickers = [t for t in OPENBB_2003_TICKERS if t not in processed_tickers]
    ...
    for ticker_idx, ticker in enumerate(tqdm(remaining_tickers, desc="Processing tickers")):
        ...
        if memory_usage > MEMORY_CRITICAL:
            ...
            print(f"CRITICAL: Memory usage at {memory_usage}%. Pausing execution...")
            wait_for_memory_to_clear(MEMORY_WARNING - 5)
            continue  # Re-check memory after waiting
        
        print(f"Processing ticker {ticker_idx + 1}/{len(remaining_tickers)}: {ticker}")
